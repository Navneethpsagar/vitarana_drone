...
# oop using class : class name is Edrone
class Edrone():
    """docstring for Edrone"""
    def __init__(self):
        rospy.init_node('attitude_controller')  # initializing ros node with name drone_control

        # 1 This corresponds to your current orientation of eDrone in quaternion format. This value must be updated each time in your imu callback
        # [x,y,z,w]
        self.drone_orientation_quaternion = [0.0, 0.0, 0.0, 0.0]

        # 2 This corresponds to your current orientation of eDrone converted in euler angles form.
        # [r,p,y]
        self.drone_orientation_euler = [0.0, 0.0, 0.0]

        # 3 This is the setpoint that will be received from the drone_command in the range from 1000 to 2000
        # [r_setpoint, p_setpoint, y_setpoint]
        self.setpoint_cmd = [0.0, 0.0, 0.0]

        # 4 The setpoint of orientation in euler angles at which you want to stabilize the drone
        ...
        #we just created a temporary container to publish what we want to do
        ...
        #these are the constants we need to find in this task
        ...
        #
        # Variable to store the previous values
        ...
        # we are publishing the things we need so we need to add these by ourselves
        ...
        # we are adding subscribers to give the command
        # message type is edrone_cmd : we need to go to a calback function to add the details
    ...
    def imu_callback(self, msg):
    	#the imu publishes the oruentation of the drone 
    	# the imu publishes the values in wuaternion and we pass on the valuse to the variables 
    ...
    def pid(self):
    	#we are going to add the PID tune package 
    	#so we need to subscribe into the rostopic ans add pid_tune and add this as a subscriber to this package
        ...
        self.throttle = 1.024*self.throttle_cmd - 1024

        # 3 Compute error in each axis. eg: error[0] = self.setpoint_euler[0] - self.drone_orientation_euler[0], where error[0] corresponds to error in roll...

        self.error_P[0] = self.setpoint_euler[0] - self.drone_orientation_euler[0]
        self.error_P[1] = self.setpoint_euler[1] - self.drone_orientation_euler[1]
        self.error_P[2] = self.setpoint_euler[2] - self.drone_orientation_euler[2]

        # 4 Compute the error (for proportional), change in error (for derivative) and sum of errors (for integral) in each axis. Refer "Understanding PID.pdf" to understand PID equation.

        #for I
        for i in range(3):
        	self.error_I[i] = self.error_I[i] + self.prev_values[i]

        #for D
        for i in range(3):
        	self.error_D[i] = (self.error_D[i] - self.prev_values[i])
        	
        for i in range(3):
        	self.prev_values[i]= error_P[i]

        for i in range(3):
        self.error_out[i] = self.Kp * self.error_P[i] + self.Ki * self.error_I[i] + self.Kd * self.error_P[i]
